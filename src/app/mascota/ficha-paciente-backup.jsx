  ...
  vacunas.forEach((vacuna) => {
    ...
    if (diff <= 7 && diff >= 0) {
      ...
      alertas.push({
        ...
        mensaje:
          diff === 0
            ...
            : `¡${vacuna.nombre_vacuna} vence en ${diff} día${
                diff > 1 ? "s" : ""
              }!`,
      });
      ...
      alertas.push({
        ...
        diasRestantes: diff,
        tipo: "vencida",
        mensaje: `¡${vacuna.nombre_vacuna} está VENCIDA desde hace ${Math.abs(
          diff
        )} día${Math.abs(diff) > 1 ? "s" : ""}!`,
      });
    }
...
export default function FichaPaciente({ mascotaId }) {
    ...
    if (mascotaId) {
      const cargarDatosMascota = async () => {
        ...
        try {
          ...
          if (
            itemsVacunasCache &&
            now - itemsVacunasCacheTime < CACHE_DURATION
          ) {
            ...
            const vacunasData = itemsData.filter((i) =>
              i.rubro?.toLowerCase().includes("vacun")
            );
          ...
          if (
            ...
            if (alertas.length > 0) {
              const tieneAlertasCriticas = alertas.some(
                (alerta) =>
  ...
  const manejarEnvioVisita = async (e) => {
    ...
    try {
      ...
      if (
        visitaForm.peso &&
        ficha?.mascota?.peso?.toString() !== visitaForm.peso.toString()
      ) {
      ...
      if (modoEdicion && visitaSeleccionada) {
        ...
        Promise.all([
          fetch(`/api/historial-mascota/${mascotaId}`).then((res) =>
            res.ok ? res.json() : null
          ),
          fetch(`/api/vacunas-aplicadas/proximas?id_mascota=${mascotaId}`).then(
          ...
          .then(([nuevoHistorial, nuevasProximasVacunas]) => {
            if (nuevoHistorial) {
              ...
              const visitaActualizada = nuevoHistorial.find(
                (v) => v.id_visita === visitaSeleccionada.id_visita
              );
    ...
    try {
      ...
      if (modoEdicionVacuna && vacunaSeleccionada) {
        ...
        const res = await fetch(
          `/api/vacunas-aplicadas/${vacunaSeleccionada.id_vacuna_aplicada}`,
          {
            method: "PUT",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
        ...
        Promise.all([
          ...
          .then(([nuevoHistorial, nuevasProximasVacunas]) => {
            if (nuevoHistorial) {
              ...
              const vacunaActualizada = nuevoHistorial
                .flatMap((v) => v.vacunas || [])
                .find(
                  (vac) =>
                    vac.id_vacuna_aplicada ===
                    vacunaSeleccionada.id_vacuna_aplicada
                );
  ...
  return (
    <div className="flex flex-col min-h-screen bg-gray-50">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
          ...
          <div className="lg:col-span-2 bg-white rounded-lg p-4">
            <h2 className="text-xl font-bold text-purple-800 mb-4">
              Historial Médico de {ficha?.mascota?.nombre}
            </h2>
            {/* Historial real */}
            {historial.length === 0 ? (
            ...
            <div className="flex justify-start space-x-2 mt-4">
              ...
              <Button
                variant="outline"
                onClick={() =>
                  visitaSeleccionada && abrirEdicionVisita(visitaSeleccionada)
                }
        ...
        <DialogContent className="max-w-2xl">
          <DialogHeader className="text-center">
            <DialogTitle className="text-2xl font-bold text-purple-800">
              {modoEdicion ? "Modificar Visita" : "Añadir Nueva Visita"}
            </DialogTitle>
            <DialogDescription className="text-gray-600">
              Complete los datos de la visita médica.
            </DialogDescription>
          </DialogHeader>
          <form className="space-y-6" onSubmit={manejarEnvioVisita}>
            ...
            <div className="text-center">
              ...
              <div className="flex justify-center">
                <Input
                  ...
                  onChange={(e) =>
                    setVisitaForm((f) => ({ ...f, fecha: e.target.value }))
                  }
                  required
            ...
            <div className="text-center">
              ...
              <textarea
                ...
                onChange={(e) =>
                  setVisitaForm((f) => ({ ...f, diagnostico: e.target.value }))
                }
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-2xl mx-auto">
              ...
              <div className="text-center">
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setVisitaForm((f) => ({ ...f, peso: e.target.value }))
                  }
                  required
                  className="w-full text-center border-gray-300 focus:ring-2 focus:ring-purple-500 focus:border-transparent"
                  placeholder="Ej: 25.5"
      ...
      <Dialog open={isVacunaDialogOpen} onOpenChange={setIsVacunaDialogOpen}>
        <DialogContent className="max-w-2xl">
          <DialogHeader className="text-center">
            ...
            <DialogDescription className="text-gray-600">
              Registre una vacuna aplicada en esta visita.
            </DialogDescription>
          </DialogHeader>
          <form className="space-y-6" onSubmit={manejarEnvioVacuna}>
            ...
            <div className="bg-gray-50 rounded-lg p-4">
              <Label
                ...
                className="text-base font-semibold text-gray-700 block mb-3"
              >
                Tipo de Vacuna
              </Label>
              {!vacunaManual ? (
                <div className="space-y-3">
                  <select
                    ...
                    onChange={(e) => {
                      if (e.target.value === "manual") {
                        ...
                        const item = itemsVacunas.find(
                          (i) => i.id_item == e.target.value
                        );
                        setDuracionEditable(false);
                        setVacunaForm((f) => ({
                          ...f,
                  ...
                  <p className="text-xs text-gray-500">
                    Seleccione una vacuna de la lista o agregue una nueva
                    manualmente
                  </p>
                </div>
              ) : (
                <div className="space-y-3">
                  ...
                  <p className="text-xs text-gray-500">
                    Ingrese el nombre de la vacuna que no está en la lista
                  </p>
                </div>
              )}
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="bg-gray-50 rounded-lg p-4">
                ...
                <p className="text-xs text-gray-500 mt-1">
                  Fecha cuando se aplicó la vacuna
                </p>
              </div>
              <div className="bg-gray-50 rounded-lg p-4">
                <Label
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setVacunaForm((f) => ({
                      ...f,
                      duracion_meses: e.target.value,
                    }))
                  }
                  required
                  className={`w-full border-gray-300 focus:ring-2 focus:ring-purple-500 focus:border-transparent ${
                    !duracionEditable ? "bg-gray-100 text-gray-500" : ""
                  }`}
                  placeholder="Ej: 12"
                ...
                {!duracionEditable && vacunaForm.duracion_meses && (
                  <p className="text-xs text-gray-500 mt-1">
                    Duración predefinida de la vacuna
                  </p>
                )}
              </div>
            </div>
            ...
            <div className="flex justify-center space-x-6 pt-6">
              <Button
                type="button"
                variant="outline"
                onClick={() => setIsVacunaDialogOpen(false)}
                className="border-gray-300 text-gray-700 hover:bg-gray-50 px-8 py-2"
              >
                Finalizar
              </Button>
      ...
      <Dialog open={isFotoDialogOpen} onOpenChange={setIsFotoDialogOpen}>
        <DialogContent className="max-w-lg">
          <DialogHeader className="text-center">
            ...
            <DialogDescription className="text-gray-600">
              Selecciona una nueva foto para la mascota.
            </DialogDescription>
          </DialogHeader>
                ...
                <div className="bg-gray-50 rounded-lg p-4">
                  ...
                  <div className="flex justify-center">
                    <ImageDisplay
                      src={nuevaFoto}
                      alt="Vista previa"
                      className="w-40 h-40 rounded-full border-2 border-purple-300 shadow-md"
                      showControls={false}
                    />
                  </div>
      ...
      <Dialog open={isDecesoDialogOpen} onOpenChange={setIsDecesoDialogOpen}>
        <DialogContent className="max-w-md">
          <DialogHeader className="text-center">
            ...
            <DialogDescription className="text-gray-600">
              Modifica el estado de vida de la mascota.
            </DialogDescription>
          </DialogHeader>
      ...
      <Dialog
        open={isNuevaMascotaDialogOpen}
        onOpenChange={setIsNuevaMascotaDialogOpen}
      >
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            ...
            <DialogDescription className="text-gray-600 text-center">
              Complete los datos de la nueva mascota para {ficha?.owner?.nombre}{" "}
              {ficha?.owner?.apellido}
            </DialogDescription>
          </DialogHeader>
          <form onSubmit={manejarEnvioNuevaMascota} className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div>
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setNuevaMascotaForm((f) => ({ ...f, raza: e.target.value }))
                  }
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) =>
                    setNuevaMascotaForm((f) => ({ ...f, sexo: e.target.value }))
                  }
                  required
                  className="w-full border border-gray-300 rounded-md p-3 focus:ring-2 focus:ring-purple-500 focus:border-transparent"
                >
              ...
              <div>
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setNuevaMascotaForm((f) => ({ ...f, edad: e.target.value }))
                  }
              ...
              <div>
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setNuevaMascotaForm((f) => ({ ...f, peso: e.target.value }))
                  }
            ...
            <div className="flex items-center space-x-2">
              <input
                ...
                onChange={(e) =>
                  setNuevaMascotaForm((f) => ({
                    ...f,
                    estado_reproductivo: e.target.checked,
                  }))
                }
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) =>
                  setNuevaMascotaForm((f) => ({ ...f, estado: e.target.value }))
                }
                className="w-full border border-gray-300 rounded-md p-3 focus:ring-2 focus:ring-purple-500 focus:border-transparent"
              >
            ...
            <div className="flex justify-center space-x-6 pt-6">
              <Button
                type="button"
                variant="outline"
                onClick={() => setIsNuevaMascotaDialogOpen(false)}
                className="border-gray-300 text-gray-700 hover:bg-gray-50 px-8 py-2"
              >
                Cancelar
              </Button>
        ...
        <div className="fixed bottom-4 right-4 z-50">
          <div
            className={`w-16 h-16 rounded-full shadow-lg cursor-pointer transition-all duration-300 hover:shadow-xl transform hover:scale-110 flex items-center justify-center ${
              alertasVacunas.some(
                (a) => a.tipo === "urgente" || a.tipo === "vencida"
              )
                ? "bg-red-500 text-white animate-pulse"
                : "bg-orange-500 text-white"
            }`}
            onClick={() => setIsAlertasModalOpen(true)}
            title={`${alertasVacunas.length} vacuna${
              alertasVacunas.length > 1 ? "s" : ""
            } pendiente${alertasVacunas.length > 1 ? "s" : ""}`}
          >
            <div className="text-center">
              <Syringe size={24} className="mx-auto mb-1" />
              <div className="text-xs font-bold">{alertasVacunas.length}</div>
            </div>
          </div>
        </div>
      )}
