...
export async function GET(request, { params }) {
  ...
  try {
    ...
    const result = await client.query(
      "SELECT * FROM vacuna_aplicada WHERE id_vacuna_aplicada = $1",
      [id]
    );
...
export async function PUT(request, { params }) {
  ...
  try {
    ...
    const { nombre_vacuna, fecha_aplicacion, duracion_meses, observaciones } =
      body;

    // Validar que los campos requeridos no sean undefined
    if (!nombre_vacuna || !fecha_aplicacion || duracion_meses === undefined) {
    ...
    console.error("Error en PUT /api/vacunas-aplicadas/[id]:", error);
    return NextResponse.json({ error: error.message }, { status: 500 });
  } finally {
    if (client) client.release();
  }
...
export async function DELETE(request, { params }) {
  ...
  try {
    ...
    const { id } = await params;
    const result = await client.query(
      "DELETE FROM vacuna_aplicada WHERE id_vacuna_aplicada = $1",
      [id]
    );
