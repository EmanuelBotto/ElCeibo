...
export async function POST(request) {
  try {
    ...
    if (!nombre || !especie || !sexo) {
      return new Response(
        JSON.stringify({ error: "Nombre, especie y sexo son requeridos" }),
        {
          status: 400,
          headers: { "Content-Type": "application/json" },
        }
    ...
    if (foto && foto.length > 7 * 1024 * 1024) {
      // ~5MB en Base64
      return new Response(
        JSON.stringify({ error: "La imagen es demasiado grande. Máximo 5MB." }),
        {
          status: 400,
          headers: { "Content-Type": "application/json" },
        }
    ...
    let fotoBuffer = null;
    if (foto) {
      console.log("Tipo de foto recibida:", typeof foto);
      console.log(
        "Contenido de foto (primeros 100 chars):",
        foto ? foto.toString().substring(0, 100) : null
      );
      ...
      try {
        ...
        if (typeof foto === "string") {
          ...
          base64Data = foto.includes(",") ? foto.split(",")[1] : foto;
        } else {
          // Si no es string, convertir a string primero
          const fotoString = foto.toString();
          base64Data = fotoString.includes(",")
            ? fotoString.split(",")[1]
            : fotoString;
        }
        ...
        console.log(
          "Foto convertida exitosamente, tamaño:",
          fotoBuffer.length,
          "bytes"
        );
      } catch (error) {
    ...
    console.log(
      "Consulta ejecutada exitosamente, ID generado:",
      result.rows[0].id_mascota
    );
    ...
    if (err.code === "23503") {
      return new Response(
        JSON.stringify({ error: "El cliente especificado no existe" }),
        {
          status: 400,
          headers: { "Content-Type": "application/json" },
        }
...
export async function PUT(request) {
  try {
    ...
    if (!id_mascota) {
      return new Response(
        JSON.stringify({ error: "ID de mascota es requerido" }),
        {
          status: 400,
          headers: { "Content-Type": "application/json" },
        }
    ...
    let fotoBuffer = null;
    if (foto) {
      console.log("PUT - Tipo de foto recibida:", typeof foto);
      ...
      try {
        ...
        fotoBuffer = Buffer.from(base64Data, "base64");
        console.log(
          "PUT - Foto convertida exitosamente, tamaño:",
          fotoBuffer.length,
          "bytes"
        );
      } catch (error) {
        console.error("PUT - Error al convertir foto:", error);
        fotoBuffer = null;
      }
    }
